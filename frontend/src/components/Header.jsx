    ...
    <header className={`sticky top-0 z-50 glass-panel border-b transition-colors duration-200 ${
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          {/* Logo & Branding */}
          <div className="flex items-center space-x-2 sm:space-x-3 cursor-pointer shrink-0" onClick={() => setMode('applicant')}>
            <div className="w-9 h-9 sm:w-11 sm:h-11 rounded-xl bg-gradient-to-tr from-amber-500 via-orange-500 to-amber-300 p-0.5 shadow-lg shadow-amber-500/20 shrink-0">
            ...
            <div>
              <div className="flex items-center space-x-1.5 sm:space-x-2">
                <span className={`text-lg sm:text-2xl font-extrabold tracking-tight font-outfit ${
                  theme === 'light' ? 'text-slate-900' : 'text-white'
                }`}>
                  {t.brandName}
                </span>
                <span className="px-1.5 py-0.5 text-[9px] sm:text-[10px] font-semibold bg-amber-500/10 text-amber-500 border border-amber-500/30 rounded-full flex items-center gap-0.5 sm:gap-1">
                  <Sparkles className="w-2.5 h-2.5 sm:w-3 sm:h-3" /> SIH 2026
                </span>
              </div>
          ...
          <div className="flex items-center space-x-1.5 sm:space-x-3 shrink-0">
            ...
            <button
              onClick={toggleTheme}
              className={`flex items-center space-x-1 sm:space-x-1.5 px-2 py-1.5 sm:px-3 sm:py-2 rounded-xl text-xs font-bold transition-all border ${
                theme === 'light'
            ...
            <div className="relative">
              <button
                ...
                className={`flex items-center space-x-1 sm:space-x-1.5 px-2 py-1.5 sm:px-3 sm:py-2 rounded-xl text-xs font-semibold border transition-all ${
                  theme === 'light'
              ...
              >
                <Globe className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-500" />
                <span className="font-bold text-[11px] sm:text-xs">{currentLangObj.native}</span>
              </button>

              {langDropdownOpen && (
                <div className={`absolute right-0 mt-2 w-44 sm:w-48 rounded-2xl border shadow-2xl py-2 z-50 ${
                  theme === 'light'
            ...
            {mode === 'applicant' ? (
              <button
                ...
                className="flex items-center space-x-1 sm:space-x-2 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-slate-950 text-[11px] sm:text-xs font-bold hover:brightness-110 transition-all shadow-lg shadow-amber-500/20 shrink-0"
              >
                <LayoutDashboard className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                <span className="hidden xs:inline">{t.navAdminMode}</span>
                <span className="xs:hidden">Admin</span>
              </button>
            ) : (
              <button
                onClick={() => setMode('applicant')}
                className={`flex items-center space-x-1 sm:space-x-2 px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-xl border text-[11px] sm:text-xs font-bold transition-all shrink-0 ${
                  theme === 'light'
              ...
              >
                ...
                <span className="hidden xs:inline">{t.navApplicantMode}</span>
