  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          <h2 className={`text-2xl font-extrabold font-outfit flex items-center gap-2 ${theme === 'light' ? 'text-slate-900' : 'text-white'
            }`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className={`lg:col-span-7 glass-panel rounded-2xl p-6 border space-y-6 ${theme === 'light' ? 'border-slate-200 bg-white' : 'border-slate-800'
          }`}>

          {/* Loan Amount Slider */}
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className={`text-xs font-bold uppercase tracking-wider ${theme === 'light' ? 'text-slate-700' : 'text-slate-300'
                }`}>
                {t.loanAmount}
              </label>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full h-2 rounded-lg appearance-none cursor-pointer accent-sky-500 ${theme === 'light' ? 'bg-slate-200' : 'bg-slate-800'
                }`}
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full h-2 rounded-lg appearance-none cursor-pointer accent-emerald-500 ${theme === 'light' ? 'bg-slate-200' : 'bg-slate-800'
                }`}
            />
            <p className={`text-[11px] mt-2 p-2.5 rounded-lg border ${theme === 'light' ? 'bg-amber-50/70 border-amber-200 text-amber-950' : 'bg-slate-900/80 border-slate-800 text-slate-400'
              }`}>
        ...
        <div className="lg:col-span-5 space-y-4">
          ...
          {/* Main EMI Card */}
          <div className={`glass-panel rounded-2xl p-6 border relative overflow-hidden shadow-2xl ${theme === 'light'
            ...
            <span className={`text-[10px] font-bold uppercase tracking-wider block mb-1 ${theme === 'light' ? 'text-slate-500' : 'text-slate-400'
              }`}>
              {t.monthlyEmi}
            </span>
            ...
            {moratoriumMonths > 0 && emiData && (
              <div className={`text-xs font-medium mb-4 p-2.5 rounded-xl border ${theme === 'light' ? 'bg-emerald-50 text-emerald-800 border-emerald-200' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20'
                }`}>
            ...
            {/* Savings Banner */}
            {emiData && (
              <div className={`p-3.5 rounded-xl border flex items-center space-x-3 mb-4 ${theme === 'light'
            ...
            {emiData && (
              <div className={`pt-3 border-t ${theme === 'light' ? 'border-slate-200' : 'border-slate-800'}`}>
                ...
                <div className={`w-full h-2 rounded-full overflow-hidden ${theme === 'light' ? 'bg-slate-200' : 'bg-slate-800'}`}>
                  <div
                    className={`h-full rounded-full transition-all ${emiData.affordability_color === 'green' ? 'bg-emerald-500' : emiData.affordability_color === 'yellow' ? 'bg-amber-500' : 'bg-rose-500'
                      }`}
